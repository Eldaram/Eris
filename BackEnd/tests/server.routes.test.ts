...
import * as socketConfig from "../src/config/socket";
...
describe("Server Routes", () => {
  ...
  beforeAll(async () => {
    ...
    const testUser = await prisma.user.create({
      data: { username: "route-test-user", pocketbaseId: "pb-route-test-user" },
    });
    testUserId = testUser.id;

    const otherUser = await prisma.user.create({
      data: {
        username: "route-other-user",
        pocketbaseId: "pb-route-other-user",
      },
    });
  ...
  afterAll(async () => {
    ...
    await prisma.user.deleteMany({
      where: { username: { in: ["route-test-user", "route-other-user"] } },
    });
  ...
  test("GET /api/servers returns only user servers", async () => {
    ...
    const s1 = await prisma.server.create({
      data: { name: "User Server 1", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: s1.id, userId: testUserId },
    });

    // create server for otherUser
    const s2 = await prisma.server.create({
      data: { name: "Other Server", ownerId: otherUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: s2.id, userId: otherUserId },
    });
  ...
  test("GET /api/servers/:serverId/channels returns channels for member and denies non-members", async () => {
    const server = await prisma.server.create({
      data: { name: "Channels Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });

    const roomA = await prisma.room.create({
      data: { name: "alpha", serverId: server.id, isDm: false },
    });
    await prisma.room.create({
      data: { name: "beta", serverId: server.id, isDm: false },
    });
    ...
    expect(resMember.body.channels.some((c: any) => c.id === roomA.id)).toBe(
      true,
    );
  ...
  test("GET /api/servers/:serverId/users returns users with online status for members", async () => {
    const server = await prisma.server.create({
      data: { name: "Users Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: otherUserId },
    });

    jest
      .spyOn(socketConfig, "isUserOnline")
      .mockImplementation((userId: string) => userId === testUserId);

    const res = await request(app)
      .get(`/api/servers/${server.id}/users`)
      .set("Authorization", "Bearer token-test-user");
  ...
  test("GET /api/servers/:serverId/users denies non-members", async () => {
    const server = await prisma.server.create({
      data: { name: "Users Access Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("GET /api/servers/:serverId/ownership returns whether the requester owns the server", async () => {
    const ownedServer = await prisma.server.create({
      data: { name: "Owned Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: ownedServer.id, userId: testUserId },
    });

    const otherServer = await prisma.server.create({
      data: { name: "Other Owned Server", ownerId: otherUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: otherServer.id, userId: otherUserId },
    });
  ...
  test("POST /api/servers/:serverId/rooms lets the owner create a room", async () => {
    const server = await prisma.server.create({
      data: { name: "Room Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("POST /api/servers/:serverId/rooms denies non-owners", async () => {
    const server = await prisma.server.create({
      data: { name: "Room Deny Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("POST /api/servers/:serverId/rooms validates the room name length", async () => {
    const server = await prisma.server.create({
      data: { name: "Room Validation Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("POST /api/servers/:serverId/invites creates an invite link for any member", async () => {
    const server = await prisma.server.create({
      data: { name: "Invite Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
    ...
    expect(res.body.inviteUrl.startsWith("http://frontend.test/invite/")).toBe(
      true,
    );
  ...
  test("POST /api/servers/:serverId/invites denies users who are not server members", async () => {
    const server = await prisma.server.create({
      data: { name: "Invite Deny Server", ownerId: otherUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: otherUserId },
    });
  ...
  test("POST /api/invites/:code/redeem adds a logged-in user once and no-ops on repeat", async () => {
    const server = await prisma.server.create({
      data: { name: "Redeem Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
    ...
    const inviteToken = new URL(createRes.body.inviteUrl).pathname
      .split("/")
      .pop();
  ...
  test("GET /api/invites/:code returns the server name and member status for confirmation", async () => {
    const server = await prisma.server.create({
      data: { name: "Preview Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("POST /api/invites/:code/redeem rejects expired invites", async () => {
    const server = await prisma.server.create({
      data: { name: "Expired Invite Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
  ...
  test("GET /api/invites/:code rejects expired invites", async () => {
    const server = await prisma.server.create({
      data: { name: "Expired Preview Server", ownerId: testUserId },
    });
    await prisma.userPerServer.create({
      data: { serverId: server.id, userId: testUserId },
    });
