...
export class ServerService {
   ...
   */
  static async createServer(input: {
    name: string;
    ownerId: string;
  }): Promise<{ id: string }> {
    ...
    try {
      ...
      try {
        NotificationService.notifyServerCreated(
          result.id,
          result.name,
          input.ownerId,
        );
        ...
        console.error(
          "Failed to send server creation notification:",
          notifError,
        );
      }
   ...
   */
  static async listServersForUser(
    userId: string,
   ...
   */
  static async listChannelsForServer(input: {
    serverId: string;
    requesterId: string;
  }) {
    ...
    if (!membership) {
      throw new ServerInputError(
        "ACCESS_DENIED",
        "Access denied to server channels.",
        403,
      );
    }
  ...
  /**
   * Returns members of a server with online presence for the requester.
   */
  static async listUsersForServer(input: {
    serverId: string;
    requesterId: string;
  }) {
    const { serverId, requesterId } = input;
    ...
    if (!membership) {
      throw new ServerInputError(
        "ACCESS_DENIED",
        "Access denied to server users list.",
        403,
      );
    }
    ...
    return memberships.map(({ user }) => ({
      ...
      isOnline: socketConfig.isUserOnline(user.id),
    }));
  }
   ...
   */
  static async isServerOwner(input: {
    serverId: string;
    userId: string;
  ...
  static async createRoom(input: {
    ...
    if (name.length > 24) {
      throw new ServerInputError(
        "INVALID_ROOM_NAME",
        "Room name must be 24 characters or fewer.",
      );
    }
  ...
  static async createInviteLink(input: {
    serverId: string;
    creatorId: string;
  }): Promise<{ inviteUrl: string; expiresAt: string; code: string }> {
    ...
    try {
      const invite = await prisma.$transaction(async (tx) => {
        ...
        if (!membership) {
          throw new ServerInputError(
            "ACCESS_DENIED",
            "Access denied to create an invite for this server.",
            403,
          );
        }
      ...
      throw new ServerInputError(
        "INVITE_CREATION_FAILED",
        "Failed to create the invitation link.",
        500,
      );
    }
  }
   ...
   */
  static async redeemInvite(input: {
    code: string;
    userId: string;
  }): Promise<{ serverId: string; joined: boolean; alreadyMember: boolean }> {
    try {
      ...
      throw new ServerInputError(
        "INVITE_REDEMPTION_FAILED",
        "Failed to join the server from the invitation link.",
        500,
      );
    }
  }
   ...
   */
  static async getInvitePreview(input: {
    code: string;
    userId: string;
  }): Promise<{ serverName: string; alreadyMember: boolean }> {
    try {
      const invite = await prisma.serverInvite.findUnique({
        where: { code: input.code },
        select: {
      ...
      throw new ServerInputError(
        "INVITE_REDEMPTION_FAILED",
        "Failed to load the invitation preview.",
        500,
      );
    }
  }
