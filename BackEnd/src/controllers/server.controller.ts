...
export class ServerController {
  ...
  static async getChannels(req: Request, res: Response) {
    ...
    if (!serverId) {
      return res
        .status(400)
        .json({ error: "serverId is required.", code: "INVALID_SERVER_ID" });
    }

    try {
      const channels = await ServerService.listChannelsForServer({
        serverId,
        requesterId: user.id,
      });
  ...
  static async getUsers(req: Request, res: Response) {
    ...
    try {
      const users = await ServerService.listUsersForServer({
        serverId,
        requesterId: user.id,
      });
      ...
      console.error("Unexpected error while listing server users:", error);
      return res.status(500).json({ error: "Internal Server Error" });
    }
  }
  ...
  static async createRoom(req: Request, res: Response) {
    ...
    if (!name || typeof name !== "string") {
      return res
        .status(400)
        .json({ error: "Room name is required.", code: "INVALID_ROOM_NAME" });
    }
