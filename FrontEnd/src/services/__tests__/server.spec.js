...
describe("serverService invite methods", () => {
  ...
  it("creates an invite with auth headers", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          inviteUrl: "http://frontend.test/invite/abc",
  ...
  it("loads invite preview", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({ serverName: "Preview Server", alreadyMember: false }),
      headers: { get: () => "application/json" },
    });
  ...
  it("creates a room in a server", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          success: true,
          room: { id: "room-1", name: "announcements", serverId: "server-1" },
        }),
      headers: { get: () => "application/json" },
    });
  ...
  it("loads server users with auth headers", async () => {
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ok: true,
      json: () =>
        Promise.resolve({
          users: [{ id: "user-1", username: "Alice", isOnline: true }],
        }),
      headers: { get: () => "application/json" },
    });

    const result = await serverService.getServerUsers("server-1");

    expect(authService.getAuthHeader).toHaveBeenCalled();
    expect(fetchSpy).toHaveBeenCalledWith(
      expect.stringContaining("/api/servers/server-1/users"),
      expect.objectContaining({
        method: "GET",
        headers: expect.objectContaining({
          Authorization: "Bearer test-token",
